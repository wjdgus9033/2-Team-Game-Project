...
import { createStart } from "./modal.js";

// --------------- 참조 함수 --------------

export function gameInitFn() {
  this.context = null;
  this.boardStatus = [];
  this.currentRound = 0;
  this.activePlayer = 1;
...
export function drawStoneFn(row, col, color) {
  const ctx = this.context;
  const cellSize = this.cellSize;
...
export function switchPlayerFn() {
  this.activePlayer = this.activePlayer === 1 ? 2 : 1;
  timerStatus.playerDefaultTime = Date.now();
}

export function gameOverFn(player) {
  this.isOver = true;
...
export function updateTimeUIFn() {
...
export function updateTurnUI(player) {
  if (player === 0) {
    return;
  }
